"""Full-length IER for Enron or C50, matching the blogs full-length protocol.

Same attributer (tuned LinearSVC), same conditions, no mid-sentence truncation,
maximum available training data per author. Resumable.

Run:  python3 -m scripts.run_corpus400 --corpus enron
...
from src.data import load_enron_rich, load_c50, to_xy
...
args = ap.parse_args()
CACHE = f"results/{args.corpus}400_rewrites.jsonl"
OUT = f"results/{args.corpus}400_ier.json"
...
print(f"[{args.corpus}] authors={len(train)} train={sum(len(v) for v in train.values())} "
      f"test={len(items)} mean_len={np.mean([len(t.split()) for t in orig]):.0f}w", flush=True)
...
res = {"corpus": args.corpus, "max_words": MAXW, "n_authors": len(train),
       "chance": 1/len(train), "baseline_acc": float(c_o.mean()), "conditions": {}}
